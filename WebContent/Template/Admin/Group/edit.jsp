...
.menuListCheckBox h2{overflow:hidden; border-bottom:1px dotted}
	...
	<div class="wrap jj">
		...
		<form method="post" action="${dfn:U('admin/group/do_edit')}" class="form-horizontal J_ajaxForm">
        <input type="hidden" name="group_id" value="${data.group_id }"/>
			<div class="common-form cwh">
				...
				<table>
					...
					<tr>
						<th width="200" class="control-label">角色名： 
						</th>
						<td><input type="text" name="groupname" value="${data.groupname }" /><span class="commonToolTip">请输入角色名</span>
						</td>
					</tr>
					<tr>
						<th class="control-label">角色描述：</th>
						<td><textarea id="groupinfo" class="myarea" name="groupinfo" cols="40" rows="5">${data.groupinfo }</textarea>
				...
				<c:if test="${not empty menuList }">
					<c:forEach items="${menuList }" var="menu_1">
                    	<!--最外层START-->
                    	<h1><em>${menu_1.name}</em><em class="ck"><input name="menuIds" value="${menu_1.id}" <c:if test="${fn:contains(hasMenu, menu_1['id'])}">checked</c:if> type="checkbox" onClick="select_all('m_1_${menu_1.id}',this);" id="m_1_${menu_1.id}" /><label for="m_1_${menu_1.id}">全选</label></em></h1>
                        
                            <c:if test="${not empty menu_1.son }">
                                <c:forEach items="${menu_1.son}" var="menu_2">
                                <ul><h2><input type="checkbox" <c:if test="${fn:contains(hasMenu, menu_2['id'])}">checked</c:if> name="menuIds" value="${menu_2.id}" class="m_1_${menu_1.id}" onClick="select_all('m_2_${menu_2.id}',this);" id="m_2_${menu_2.id}" /><label for="m_2_${menu_2.id}">${menu_2.name}</label></h2>
                                
                                    <c:if test="${not empty menu_2.son }">
                                        <c:forEach items="${menu_2.son}" var="menu_3">
                                            <li><span><input name="menuIds" value="${menu_3.id}" <c:if test="${fn:contains(hasMenu, menu_3['id'])}">checked</c:if> type="checkbox" class="m_1_${menu_1.id} m_2_${menu_2.id}" onClick="select_all('m_3_${menu_3.id}',this);" id="m_3_${menu_3.id}" /><label for="m_3_${menu_3.id}">${menu_3.name}</label></span>
                                            <c:if test="${not empty menu_3.son }">
                                                <c:forEach items="${menu_3.son}" var="menu_4">
                     <b><input name="menuIds" value="${menu_4.id}" type="checkbox" <c:if test="${fn:contains(hasMenu, menu_4['id'])}">checked</c:if> class="m_1_${menu_1.id} m_2_${menu_2.id} m_3_${menu_3.id}" onClick="select_all('m_4_${menu_4.id}',this);"  id="m_${menu_4.id}" /><label for="m_${menu_4.id}">${menu_4.name}</label></b>
                                                </c:forEach>
                                            </c:if>
