...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<div class="nav">
			<ul class="cc">
				<li class="current"><a href="${dfn:U('admin/mortgage/index')}">信用标配置</a></li>
			</ul>
		</div>
		...
		<form class="form-horizontal J_ajaxForm" name="myform" id="myform"
			action="${dfn:U('admin/mortgage/editMortgage')}" method="post">
			<dfTag:token />
			<fieldset>
				<div class="tabbable">
									...
									<tbody>
										<c:if test="${not empty data }">
											<c:forEach items="${data['yt']}" var="user"
												varStatus="status">
												<tr overstyle="on" id="list_yt_${user['value']['key']}">
								...
								<table width="100%" class="table_form">
									 <tr>
										...
										<td><input type="radio"
											...
											for="borrow[borrowFee][borrowManageType]_1">统一资费</label>
											...
											onClick="feeType(this);"> <label
											for="borrow[borrowFee][borrowManageType]_2">按还款方式分别设置</label>
									...
									<tr class="feeTypeOther">
										<th width="200" class="control-label">按季分期还款(分别设置)：</th>
										<td>${dfForm:input( "borrow[borrowFee][borrowManageSeason]", data['borrow']['borrowFee']['borrowManageSeason'], dfForm_class, dfForm_style,dfForm_others)}<span
