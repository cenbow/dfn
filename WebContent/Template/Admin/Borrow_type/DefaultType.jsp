...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<div class="nav">
			<ul class="cc">
				<li class="current"><a href="${dfn:U("admin/borrow_type/index")}?type=normal">信用标配置</a></li>
			</ul>
		</div>
		...
		<form class="form-horizontal J_ajaxForm" name="myform" id="myform" action="${dfn:U('admin/borrow_type/save')}" method="post">
			...
			<fieldset>
				<div class="tabbable">
					<div class="tab-content">
							...
							<div class="table_full">
								<table width="100%" class="table_form">
									<tr>
										...
										<td><input type="radio"	name="borrow[borrowFee][borrowManageType]" value="1" <c:if test="${data['borrow']['borrowFee']['borrowManageType'] ==1 }"> checked </c:if>id="borrow[borrowFee][borrowManageType]_1" onClick="feeType(this);">
                                        <label for="borrow[borrowFee][borrowManageType]_1" style="display:inline">统一资费</label>
                                        <input	type="radio"  <c:if test="${data['borrow']['borrowFee']['borrowManageType'] ==2 }"> checked </c:if>	name="borrow[borrowFee][borrowManageType]" value="2" id="borrow[borrowFee][borrowManageType]_2" onClick="feeType(this);">
                                        <label	for="borrow[borrowFee][borrowManageType]_2" style="display:inline">按还款方式分别设置</label>
										<span class="commonToolTip">收费方式(borrowManageType)</span></td>
									</tr>
						...
						<script type="text/javascript">
							<c:if test="${data['borrow']['borrowFee']['borrowManageType'] ==2 }">feeType(false,2)</c:if>
							<c:if test="${data['borrow']['borrowFee']['borrowManageType'] ==1 }">feeType(false,1)</c:if>
							function feeType(obj,val) {
								var type = (obj)?$(obj).val():val;
								if (type == 2) {
									$(".feeTypeSame").hide();
									$(".feeTypeOther").show();
								} else {
