			...
			<div class="login_con_box">
				<div class="lg_box_l fl">
					<form>
						<fieldset>
							<div class="lg_text1">
								<label>帐号：</label><br/><input type="text" name="user_name"	 class="textput" id="user_name" title="请输入6-20字符的用户名，中文占2个字符" />

								<!--  <input type="text" /> -->
							</div>
							<div class="lg_text1">
								<label>密码：</label><br/><input type="password" name="pass_word" id="pass_word" class="textput" title="请输入大于6个字符的登陆密码" />

								<!--  <input type="text" /> -->
							</div>
                            <div class="reg_yanzheng clear clearfix pb10 lg_text1">
                                <label>验证码：</label><br/><input type="text" name="vcode" id="vcode" placeholder="请输入右图中验证码" class="textput fl" style="margin-left: 0px;" /> <img src="${dfn:U('home/login/verifyCode')}" width="115px" style="cursor:pointer" onclick="javascript:this.src='${dfn:U('home/login/verifyCode')}?time='+Math.random();void(0);"  />
                            </div>
							<div class="lg_text2 clear clearfix">
								<span><input type="checkbox" id="isrember" value="1" />1天内自动登陆</span> <a
									href="${dfn:U('home/getpassword/index')}">忘记密码？</a>
							</div>
...
function login(){
	...
	if($("#isrember").is(":checked")) p['isrember']=1;
	Mix.load('登陆中......');
	postData("${dfn:U('home/login/doLogin')}",p,function(d){
		if(d.status==1){
			Mix.msg(d.msg,"success",2);
			setTimeout(function(){ window.location.href="${_WEB_ROOT_}/member";},1000);
		}else{
