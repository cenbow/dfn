...
//针对js中浮点儿计算中加法、减法以及取余数失去精度问题 
//  *  由于很多时候都会设计到浮点小数的算法，   *  在JS 中只用普通的parseFlost之类的进信封数据类型转换会使数据失去精度   *  因此采用先转整数再计算的方式   *   * / / / 浮点数相加  
...
//较为精确的除法结果。 
//调用：accDiv(arg1,arg2) //返回值：arg1除以arg2的精结果 
...
function toDecimal(datevalue){  
	if(datevalue.indexOf('%') != -1){  
		...
		if (datevalue.indexOf(',')  !=   - 1)  {         
			datevalue  =  datevalue.replace(/,/g, '');        
		}                // 除100精度在原有基础上增加2位。   var decimal = (datevalue.indexOf('.') == -1) ? 0 : (datevalue.length - datevalue.indexOf('.') - 1);  
	        datevalue  =  accDiv(datevalue,  100).toFixed(decimal  +  2);  //     alert("toDecimal: " + datevalue);     } else {  
		if (datevalue.indexOf(',')  !=   - 1) { 
			        datevalue  =  datevalue.replace(/,/g, '');        
		}     
	}     
