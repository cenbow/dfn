...
window.layer = {
    ...
    msg: function(msgText, msgTime, parme, callback){
        ...
        if(typeof parme === 'number'){
			conf.className='dfp2p_loading_'+parme;
            icon = parme;
        } else {
            parme = parme || {};
            icon = parme.type;
    ...
    load: function(loadTime , loadgif , loadShade, loadmsg){
       if(typeof loadmsg === 'string'){
            return this.msg(loadmsg, 0, 16);
        } else {
            return $.layer({
                time: loadTime,
                loading: {type : loadgif},
                bgcolor: !loadgif ? '' : '#fff',
                shade: [0.1, '#000', !loadgif ? false : true],
                border :[7,0.3, '#000', (loadgif === 3 || !loadgif) ? false : true],
                type : 3,
                title : ['',false],
                closeBtn : [0 , false]
            });
...
Class.pt.space = function(html){
    var html = html || '', times = this.index, config = this.config,className = config.className, dialog = config.dialog, dom = this.dom,
    ico = dialog.type === -1 ? '' : '<span class="xubox_msg xulayer_png32 xubox_msgico xubox_msgtype' + dialog.type + '"></span>',
    frame = [
    '<div class="xubox_dialog '+className+'">'+ ico +'<span class="xubox_msg xubox_text" style="'+ (ico ? '' : 'padding-left:20px') +'">' + dialog.msg + '</span></div>',	
