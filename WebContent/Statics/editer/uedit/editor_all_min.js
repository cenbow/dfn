...
//        'num2' : 'num-3-',
    ...
    me.ready(function () {
        ...
        //如果不给宽度会在自定应样式里出现滚动条
        utils.cssRule('list', customCss.join('\n'), me.document);
    });
...
//            }
            ...
            //node.style.cssText = style ? 'list-style-type:' + style : '';//取消自动调整ul样式style属性的功能
            //node.className = utils.trim(node.className.replace(/list-paddingleft-\w+/,'')) + ' list-paddingleft-' + type;
            utils.each(domUtils.getElementsByTagName(node,'li'),function(li){
                //li.style.cssText && (li.style.cssText = '');//取消自动调整li样式style属性的功能
                if(!li.firstChild){
                    domUtils.remove(li);
                    return;
                }
    ...
    UE.plugins['source'] = function (){
        ...
        me.queryCommandState = function (cmdName){
            ...
            if (sourceMode) {
                ...
                return (cmdName in cmdReturn)? (cmdReturn[cmdName]): -1
            }
...
UE.plugins['serialize'] = function () {
    ...
    var me = this, autoClearEmptyNode = me.options.autoClearEmptyNode,notAutoClearEmptyNodeObj=me.options.notAutoClearEmptyNodeObj,
        EMPTY_TAG = dtd.$empty,
        parseHTML = function () {
    ...
    function transNode( node, word_img_flag ) {
        ...
        switch ( node.tag ) {
            ...
            case 'span':
                ...
                if ( utils.isEmptyObject( node.attributes ) && autoClearEmptyNode && !("span" in notAutoClearEmptyNodeObj)) {
                    node.type = 'fragment'
                }
                break;
            case 'font':
...
UE.plugins['usehtml'] = function () {
        var me = this;
        var usehtml = UE['usehtml'] = 1;
        me.commands['usehtml'] = {
            queryCommandState: function (){
				UE['usehtml'] = usehtml;
                return usehtml;
...
///commandsTitle  应用样式
///commandsDialog  dialogs\template
UE.plugins['selectstyle'] = function () {
    UE.commands['selectstyle'] = {
        execCommand:function (cmd, obj) {
			var headobj=this.document.getElementsByTagName('head'); 
			var bodyobj=this.document.body;
			var fileref=this.document.createElement("link");
			var links = headobj[0].getElementsByTagName('link');
			for(j in links){
				if(typeof links[j].title!="undefined" && links[j].title=="selectstyle"){
					links[j].parentNode.removeChild(links[j]);
				}
			}

			bodyobj.className=obj.bodycss;

			fileref.rel="stylesheet"; 
			fileref.type="text/css"; 
			fileref.href=obj.url; 
			fileref.title="selectstyle"; 
			headobj[0].appendChild(fileref); 
			//obj.html && this.execCommand("inserthtml", obj.html);
        }
    };
    this.addListener("click", function (type, evt) {
...
(function () {
    ...
    var iframeUrlMap = {
		...
		'selectstyle':'~/dialogs/selectstyle/selectstyle.html'
    };
    ...
    var btnCmds = ['undo', 'redo', 'formatmatch',
        ...
        'deletecol', 'splittorows', 'splittocols', 'splittocells', 'mergecells', 'deletetable',"selectstyle","usehtml"];
    ...
    var dialogBtns = {
        ...
        ok:['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage',
            'insertvideo', 'highlightcode', 'insertframe','edittip','edittable', 'edittd','scrawl', 'template','music', 'background','selectstyle']
