var _topWin = window.dialogArguments || opener || parent || top;
;(function () {
    //全局ajax处理
    $.ajaxSetup({
    ...
    function setBtnWrap(reset) {
		var fix=(typeof _fixBtn=="undefined")?false:_fixBtn;
        if (parent.Wind && parent.Wind.art) {
            //过滤弹窗
            return;
        }

        if ($('body').height() <= $(window).height() && !fix) {
    ...
    if (ajaxForm_list.length) {
        Wind.use('ajaxForm', 'artDialog', function () {
            ...
            $('button.J_ajax_submit_btn').on('click', function (e) {
                e.preventDefault();
			    //表单获取内容前操作
			    if(typeof _beforeGetValue!="undefined"){
					var _beforeRes = _beforeGetValue(e);
					if(_beforeRes===false) return false;
				}
               /*var btn = $(this).find('button.J_ajax_submit_btn'),
                ...
                form.ajaxSubmit({
                    ...
                    beforeSubmit: function (arr, $form, options) {
                        var text = btn.text();
						//表单提交前操作
						if(typeof _beforeSubmit!="undefined"){
							var _beforeSubmitResult = _beforeSubmit(arr);
							if(_beforeSubmitResult===false) return false;
						}
                        //按钮文案、状态修改
                    ...
                    success: function (data, statusText, xhr, $form) {
                        ...
                        if (data.state === 'success') {
							if(typeof _afterSuccess!="undefined") _afterSuccess(data,btn);//成功后的自定义操作
							else{
						  ...
						  //提交失败后操作
						  if(typeof _afterFail!="undefined") _afterFail(data);
						  else{
    ...
    if ($('a.J_ajax_del').length) {
        Wind.use('artDialog','iframeTools', function () {
            $('.J_ajax_del').live('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
                        ...
                        $.getJSON(href).done(function (data) {
                            if (data.state === 'success') {
								if(typeof data.info!="undefined"){
									try{ $("#list_"+data.info).remove();}catch(e){};
								}else{
									if (data.referer) {
    ...
    if ($('a.J_ajax_confirm').length) {
        Wind.use('artDialog','iframeTools', function () {
            $('.J_ajax_confirm').live('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
                    ...
                    ok: function () {
                        $.getJSON(href).done(function (data) {
                            if (data.state === 'success') {
								if(typeof data.info!="undefined"){
									art.dialog.alert(data.info);
									setTimeout(function(){reloadPage(window)},2000);
								}else{
									if (data.referer) {
										location.href = data.referer;
									} else {
...
function reloadPage(win) {
    ...
    if(typeof win._CurPageParm!="undefined"&&win._CurPageParm!="") location.href = location.pathname+win._CurPageParm;
	else location.href = location.pathname + location.search;
}
...
//打开新窗口
function newiframe(id,href,str){
	ele = $('<span style="display:none">'+str+'</span>').appendTo("body");
    _topWin.iframeJudge({
        elem: ele,
        href: href,
        id: id
...
function dialogUrl(linkurl, title, close_type,w, h, id){
	var id = id || (Math.random(0,1)*100) ;
	omnipotent(id, linkurl, title, close_type, w, h);
}
//弹出对话框
function omnipotent(id, linkurl, title, close_type, w, h) {
    ...
    var h = h||300;
    Wind.use("artDialog","iframeTools",function(){
        currentDialog = art.dialog.open(linkurl, {
        id: id,
        title: title,
        width: w,
        height: h,
        ...
        button: [/*{
            ...
            callback: function () {
				if(typeof dialogCallBack=="function"){
					return dialogCallBack(close_type,id);
				}
				else{
					if (close_type == 1) {
						return true;
