...
$(function () {
    ...
    $("#refresh_wrapper").click(function(){
    	...
    	$loading.show();
		var frameID  = $current_iframe.attr("id");
    	//$current_iframe.attr("src",$current_iframe.attr("src"));
		//alert(window.frames[frameID]._currentUrl);
		if(typeof window.frames[frameID]._CurPageParm!="undefined" && window.frames[frameID]._CurPageParm!=""){
			var localUrl = window.frames[frameID].location.href;
			var thisUrl = (localUrl.indexOf("?")>0)?localUrl.substr(0,localUrl.indexOf("?")):localUrl;
			window.frames[frameID].location.href=thisUrl+""+window.frames[frameID]._CurPageParm;
		}else if(typeof window.frames[frameID]._currentUrl!="undefined" && window.frames[frameID]._currentUrl!="") window.frames[frameID].location.href=window.frames[frameID]._currentUrl;
		else window.frames[frameID].location.reload();
    	return false;
    });
...
function closeapp($this){
	if(!$this.is(".noclose")){
		frameId = "appiframe-"+$($this).attr("app-id");
		$("#"+frameId).remove();
		$this.prev().click();
    	$this.remove();
    	calcTaskitemsWidth();
...
function openapp(url, appid, appname, selectObj) {
    ...
    if ($app.length == 0) {
    	$(".this_left_menu_current").removeClass('this_left_menu_current');
		$("#_li_"+appid).addClass("this_left_menu_current");
        ...
        $(".appiframe").hide();
        $loading.show();
        $appiframe=$(appiframe_tpl).attr("src",url).attr("id","appiframe-"+appid).attr("name","appiframe-"+appid);
		...
		$(".this_left_menu_current").removeClass('this_left_menu_current');
		$("#_li_"+appid).addClass("this_left_menu_current").parents(".submenu").show();
		$("#_li_"+appid).parent(".submenu").parent("li").addClass("open").parent(".submenu").parent("li").addClass("open");
    	//window.frames["appiframe-"+appid].src=url;
		//window.frames["appiframe-"+appid].location.reload();
    	/*if(src!=GV.HOST+url){
    		$loading.show();
    		$iframe.attr("src",url);
    		$appiframe.load(function(){
