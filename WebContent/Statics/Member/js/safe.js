...
itz.safe = {
    ...
    /*    $.validator.addMethod("trimEmail",
        ...
        case "realname":
            "0" == safeData.user.realStatus && (this.realName.bind(safeData), this.bindedObj.realName = 1, this.realName.open());
            break;
        case "phone":
    ...
    bindBtns: function(safeData) {
        ...
        function() {
            ...
            if (!$that.hasClass(".user-safety-options-state-false")) {
                ...
                switch ($that.attr("id")) {
                case "realnameBtn2":
                    util.promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "修改实名认证需要联系客服", '您可以联系 <a href="http://qiao.baidu.com/v3/?module=default&amp;controller=webim&amp;action=index&amp;siteid=2486763&amp;groupid=0&amp;groupname=%E6%8A%95%E8%B5%84%E5%92%A8%E8%AF%A2" target="_blank">【在线客服】</a><br/>或拨打：400-0088-100，联系客服修改', 2]);
                    break;
                case "realnameBtn1":
                ...
                case "urgentBtn1":
                case "urgentBtn2":
                    binded.urgent || (objThis.urgent.set(safeData), binded.urgent = 1),
                    objThis.basicBtnEvent($that,
                    function() {
                        objThis.urgent.open(safeData)
                    },
                    function() {
                        objThis.urgent.close()
                    })
                }
            }
        }),
        ...
        bind: function(safeData) {
            ...
            $realApiCon.find("form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    return 0 !== idCardInfo.code ? void promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "您的身份证号不合法~", "请仔细检查并认真输入~<br/>", 0]) : idCardInfo.data.if18 ? ($submitBtn.attr("disabled", !0).val("提交中..."), void $.ajax({
                        ...
                        error: function() {
                            promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "由于网络原因，验证失败！", "您可以点击按钮重试，或联系客服：400-0088-100<br/>", 0]),
                            $submitBtn.removeAttr("disabled").val("提交实名认证")
                        }
                    })) : void promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "您未满18周岁，暂不可以投资喔~", "满18周岁后再来吧~<br/>", 0])
                },
                rules: {
                    realname: {
                        required: !0,
            ...
            $realUploadCon.find("form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    if ("1" == cardType) {
                        if (idCardInfo = that.getInfoByIdCard(cardValue, safeData.st), 0 !== idCardInfo.code) return void promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "您的身份证号不合法~", "请仔细检查并认真输入~<br/>", 0]);
                        if (!idCardInfo.data.if18) return void promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "您未满18周岁，暂不可以投资喔~", "满18周岁后再来吧~<br/>", 0])
                    }
                    return $("#card_pic1").val() ? $("#card_pic2").val() ? uploadStatus1 || uploadStatus2 ? void promptA("Drawings_prompt", "promptTmpl", ["实名认证提示", "正在上传图片中……", "请稍后再提交~<br/>", 0]) : ($submitBtn.attr("disabled", !0).val("提交中..."), void $.ajax({
                        url: safeData.realnameApi,
                        type: "post",
                        dataType: "json",
                    ...
                    })) : void $("#cardError2").text("请上传证件的背面~") : void $("#cardError1").text("请上传证件的正面~")
                },
                rules: {
                    realname: {
                        required: !0,
            ...
            $cardUpload1.fileupload({
                ...
                add: function(e, data) {
                    if (1 !== uploadStatus1) {
                        ...
                        if (!/(\.|\/)(gif|jpe?g|png)$/i.test(thisFile.name)) return void $cardError1.text("上传失败，只能上传后缀为gif|jpeg|png的图片喔~");
                        if (thisFile.size > 2097152) return void $cardError1.text("上传失败，文件超过2m了喔~");
            ...
            $cardUpload2.fileupload({
                ...
                add: function(e, data) {
                    if (1 !== uploadStatus2) {
                        ...
                        if (!/(\.|\/)(gif|jpe?g|png)$/i.test(thisFile.name)) return void $cardError2.text("上传失败，只能上传后缀为gif|jpeg|png的图片喔~");
                        if (thisFile.size > 2097152) return void $cardError2.text("上传失败，文件超过2m了喔~");
    ...
    phone: {
        bind: function(safeData) {
            var tempUrl, $cDown, $phoneCon1 = $("#phoneCon1"),
            $sendBtn = $("#sendSmsBtn");
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (0 === data.code) {
                                ...
                                safe = itz.safe;
                                "1" == $step ? ($item.find(".icon-tanhao").removeClass("icon-tanhao").addClass("icon-check"), $item.find(".user-safety-options-value").text(data.data.maskphone), $item.find("#phoneBtn1").text("收起"), $phoneCon1.hide(), safe.statusShow($("#phoneCon2"), "恭喜您成功认证手机号"), $item.find("#phoneNewText").text("原手机号码"), $phoneCon1.attr("_step", "2"), safe.user.phoneStatus = 1, $cDown.reset(), $sendBtn.unbind("click"), $cDown = $sendBtn.itzCutDownBtn($("#phoneNum"), safeData.getSmsVcode)) : "2" == $step ? (safe.GoStep($statusSpan, 2), $item.find("form")[0].reset(), $item.find("#phoneNewText").text("新手机号码"), $item.find(".js_vnum").val("2"), $phoneCon1.attr("_step", "3"), $cDown.reset()) : (safe.GoStep($statusSpan, 3), $item.find(".user-safety-options-value").text(data.data.maskphone), $phoneCon1.hide(), safe.statusShow($("#phoneCon2"), "恭喜您成功认证新手机"), $item.find("#phoneBtn2").text("收起"), $item.find("#phoneNewText").text("原手机号码"), $phoneCon1.attr("_step", "2"), $cDown.reset())
                            } else "登录超时，请重新登录" == data.info ? itz.util.promptA("Drawings_prompt", "promptTmpl", ["手机认证提示", "爱亲，验证失败！", "原因是：" + data.info + '<br/><a href="/newuser/index/login">重新登录</a>', 0]) : itz.util.promptA("Drawings_prompt", "promptTmpl", ["手机认证提示", "爱亲，验证失败！", "原因是：" + data.info + "<br/>", 0]),
                            $phoneCon1.find("input[name=sms_vcode]").val("");
                            $submitBtn.removeAttr("disabled").val("下一步")
                        },
                        error: function() {
                            itz.util.promptA("Drawings_prompt", "promptTmpl", ["手机认证提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                            $submitBtn.removeAttr("disabled").val("下一步")
                        }
                    })
                },
        ...
        close: function() {
            ...
            if ($w.slideUp().find("form").each(function() {
                this.reset()
            }), $con.hide(), $("#phoneCon2").hide(), "0" == itz.safe.user.phoneStatus) $("#phoneBtn1").removeClass("safe-off").attr("switchtext", "取消认证").text("认证");
            else {
                var $btn1 = $("#phoneBtn1");
                ...
                $emailCon1.find("form").validate({
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            error: function() {
                                itz.util.promptA("Drawings_prompt", "promptTmpl", ["邮箱验证提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                                $submitBtn.removeAttr("disabled").val("发送验证邮件")
                            }
                        })
                    },
                    ...
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (0 === data.code) {
                                    {
                                        $emailCon2.parents(".user-safety-options-item")
                                    }
                                    $emailCon2.find("#emailCon21").hide(),
                                    $("#gotoEmailBtn2").attr("href", that.getEmailUrl($(form).find("input[name=email]").val())),
                                    $emailCon2.find("#emailCon22").fadeIn(),
                                    $("#emailBtn2").text("收起")
                                } else itz.util.promptA("Drawings_prompt", "promptTmpl", ["邮箱验证提示", "爱亲，邮箱验证失败了！", "原因是：" + data.info + "<br/>", 0]);
                                $submitBtn.removeAttr("disabled").val("发送验证邮件")
                            },
                            error: function() {
                                itz.util.promptA("Drawings_prompt", "promptTmpl", ["邮箱验证提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                    ...
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (0 === data.code) {
                                    {
                                        $pwdCon1.parents(".user-safety-options-item")
                                    }
                                    $pwdCon1.hide(),
                                    itz.safe.statusShow($("#pwdCon2"), "恭喜您成功修改密码"),
                                    $("#pwdBtn1").text("收起"),
                                    $.getQueryString("retUrl") && setTimeout(function() {
                                        location.href = $.getQueryString("retUrl")
                                    },
                                    2e3)
                                } else itz.util.promptA("Drawings_prompt", "promptTmpl", ["登录密码提示", "爱亲，修改失败！", "原因：" + data.info + "<br/>", 0]);
                                $submitBtn.removeAttr("disabled").val("修改登录密码")
                            },
                            error: function() {
                                itz.util.promptA("Drawings_prompt", "promptTmpl", ["登录密码提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                                $submitBtn.removeAttr("disabled").val("修改登录密码")
                            }
                        })
                    },
    ...
    payPwd: {
                    ...
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                } else itz.util.promptA("Drawings_prompt", "promptTmpl", ["设置支付密码提示", "爱亲，设置失败！", "原因：" + data.info.replace("支付密码", "原支付密码") + "<br/>", 0]);
                                $submitBtn.removeAttr("disabled").val("设置支付密码")
                            },
                            error: function() {
                                itz.util.promptA("Drawings_prompt", "promptTmpl", ["设置支付密码提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                                $submitBtn.removeAttr("disabled").val("设置支付密码")
                            }
                        })
                    },
        ...
        step2: function(url) {
            ...
            $ppCon2.find("#findPayPwdBtn").click(function() {
                return "0" == itz.safe.user.phoneStatus ? void itz.util.promptA("Drawings_prompt", "promptTmpl", ["找回密码提示", "爱亲，请您先进行手机认证！", '点击进行<a href="/newuser/main/safe?curMod=phone">手机认证</a><br/>', 2]) : ($ppCon2.hide(), $("#payPwdBtn2").text("收起"), void $("#payPwdCon3").fadeIn().find(".yuyin-channel").hide())
            });
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (0 === data.code) {
                                {
                                    $ppCon2.parents(".user-safety-options-item")
                                }
                                $ppCon2.hide(),
                                itz.safe.statusShow($("#payPwdCon4"), "恭喜您成功修改支付密码"),
                                $("#payPwdBtn2").text("收起")
                            } else itz.util.promptA("Drawings_prompt", "promptTmpl", ["修改支付密码提示", "爱亲，设置失败！", "原因：" + data.info + "<br/>", 0]);
                            $submitBtn.removeAttr("disabled").val("修改支付密码")
                        },
                        error: function() {
                            itz.util.promptA("Drawings_prompt", "promptTmpl", ["修改支付密码提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                            $submitBtn.removeAttr("disabled").val("修改支付密码")
                        }
                    })
                },
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (0 === data.code) {
                                {
                                    $ppCon3.parents(".user-safety-options-item")
                                }
                                $ppCon3.hide(),
                                itz.safe.statusShow($("#payPwdCon4"), "恭喜您成功修改支付密码")
                            } else itz.util.promptA("Drawings_prompt", "promptTmpl", ["修改支付密码提示", "爱亲，设置失败！", "原因：" + data.info + "<br/>", 0]);
        ...
        open: function() {
            ...
            safe.user.paypassword ? ($w.find("#payPwdCon2").show(), $("#payPwdBtn2").addClass("safe-off").attr("switchtext", "修改").text("取消修改")) : ($w.find("#payPwdCon1").show(), $("#payPwdBtn1").addClass("safe-off").attr("switchtext", "设置").text("取消设置"))
        },
        forgetOpen: function() {
            if ("0" == itz.safe.user.phoneStatus) return void itz.util.promptA("Drawings_prompt", "promptTmpl", ["找回密码提示", "爱亲，请您先进行手机认证！", '点击进行<a href="/newuser/main/safe?curMod=phone">手机认证</a><br/>', 2]);
            var $w = $("#payPwdWrapper"),
            safe = itz.safe;
            $w.slideDown(),
            safe.manageObj.payPwd = 1,
        ...
        close: function() {
            ...
            safe.user.paypassword ? ($w.find("#payPwdCon3").hide(), $w.find("#payPwdCon4").hide(), $("#payPwdBtn2").removeClass("safe-off").attr("switchtext", "取消修改").text("修改")) : ($("#payPwdBtn1").removeClass("safe-off").attr("switchtext", "取消设置").text("设置"), $w.find("#payPwdCon1").hide(), $w.find("#payPwdCon3").hide())
        }
    },
    question: {
        step1: function(url) {
                ...
                $qCon1.find("form").validate({
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            error: function() {
                                itz.util.promptA("Drawings_prompt", "promptTmpl", ["密码保护提示", "爱亲，由于网络原因，提交失败！", "您可以点击添加重试，或联系客服：400-0088-100<br/>", 0]),
                                $submitBtn.removeAttr("disabled").val("设置")
                            }
                        })
                    },
        ...
        open: function() {
            ...
            safe.user.question ? ($w.find("#questionCon2").show(), $("#questionBtn2").addClass("safe-off").attr("switchtext", "修改").text("取消修改")) : ($w.find("#questionCon1").show(), $("#questionBtn1").addClass("safe-off").attr("switchtext", "设置").text("取消设置"))
        },
        close: function() {
            var $w = $("#quesWrapper"),
            safe = itz.safe;
            ...
            safe.user.question ? ($w.find("#questionCon1").hide(), $w.find("#questionCon3").hide(), $w.find("#questionCon4").hide(), $("#questionBtn2").removeClass("safe-off").attr("switchtext", "取消修改").text("修改")) : $("#questionBtn1").removeClass("safe-off").attr("switchtext", "取消设置").text("设置")
        }
    },
    urgent: {
        set: function(safeData) {
            var $w = $("#urgentWrapper");
            $w.find("#urgent-findPayPwdBtn").click(function() {
                return "0" == itz.safe.user.phoneStatus ? void itz.util.promptA("Drawings_prompt", "promptTmpl", ["找回密码提示", "爱亲，请您先进行手机认证！", '点击进行<a href="/newuser/main/safe?curMod=phone">手机认证</a><br/>', 2]) : ($w.hide(), $(window).scrollTop(662), $("#urgentBtn1").removeClass("safe-off").attr("switchtext", "取消设置").text("设置"), $("#urgentBtn2").removeClass("safe-off").attr("switchtext", "取消修改").text("修改"), $("#payPwdWrapper").slideDown(), $("#payPwdBtn2").addClass("safe-off").text("收起"), $("#payPwdCon2").hide(), $("#payPwdCon3").fadeIn().find(".yuyin-channel").hide(), void itz.safe.payPwd.step3(safeData))
            }),
            $w.find("input[name=name]").on("keyup",
            function() {
                $w.find(".name-error").text(""),
                $w.find(".sub-error").text("")
            }),
            $w.find("input[name=phone]").on("keyup",
            function() {
                $w.find(".phone-error").text(""),
                $w.find(".sub-error").text("")
            }),
            $w.find("select[name=relationship]").on("change",
            function() {
                $w.find(".sub-error").text("")
            }),
            $w.find("input[name=paypassword]").on("keyup",
            function() {
                $w.find(".pay-error").text("")
            });
            $w.find("form").validate({
                submitHandler: function(form) {
                    $w.find(".form-style-1-error").text("");
                    var $submitBtn = $("#urgentSubmitBtn"),
                    txt = safeData.user.urgent ? "修改": "设置";
                    $submitBtn.attr("disabled", !0).val(txt + "中..."),
                    $.ajax({
                        url: safeData.urgentApi,
                        type: "post",
                        dataType: "json",
                        data: $(form).serialize(),
                        success: function(data) {
                            if (0 === data.code) {
                                safeData.user.urgent = 1;
                                for (var prefix = ($w.parents(".user-safety-options-item"), ""), i = 0; i < $(form).find("input[name=name]").val().length - 1; i++) prefix += "*";
                                $w.find(".form-style-1-field").hide().end().find(".form-style-1-submit").hide().end().prev().find(".user-safety-options-value").remove().end().find(".icon-tanhao").removeClass("icon-tanhao").addClass("icon-check").end().find(".user-safety-options-state").before('<span class="user-safety-options-value">' + prefix + $(form).find("input[name=name]").val().slice( - 1) + '<span style="display:inline-block;width:100px;"></span>' + $(form).find("input[name=phone]").val().slice(0, 3) + "****" + $(form).find("input[name=phone]").val().slice( - 4) + "</span>"),
                                itz.safe.statusShow($("#urgentSucceed"), "恭喜您成功" + txt + "紧急联系人"),
                                $("#urgentBtn1").text("收起").attr("id", "urgentBtn2"),
                                $("#urgentBtn2").text("收起")
                            } else switch (data.code) {
                            case 1:
                                $w.find(".name-error").text(data.info);
                                break;
                            case 2:
                                $w.find(".phone-error").text(data.info);
                                break;
                            case 3:
                                $w.find(".relation-error").text(data.info);
                                break;
                            case 4:
                                $w.find(".pay-error").text(data.info);
                                break;
                            case 6:
                                $w.find(".sub-error").text(data.info);
                                break;
                            case 5:
                                itz.util.promptA("Drawings_prompt", "promptTmpl", [txt + "紧急联系人提示", "爱亲，设置失败！", "原因：" + data.info + "<br/>", 0])
                            }
                            $submitBtn.removeAttr("disabled").val("确定")
                        },
                        error: function() {
                            itz.util.promptA("Drawings_prompt", "promptTmpl", [txt + "紧急联系人提示", "爱亲，由于网络原因，提交失败！", "您可以点击重试，或联系客服：400-0088-100<br/>", 0]),
                            $submitBtn.removeAttr("disabled").val("确定")
                        }
                    })
                },
        ...
        open: function(safeData) {
            var $w = $("#urgentWrapper"),
            safe = itz.safe;
            if (0 == itz.safe.user.phoneStatus) {
                $("#urgentBtn1").removeClass("safe-off").attr("switchtext", "取消设置").text("设置"),
                $("#urgentBtn2").removeClass("safe-off").attr("switchtext", "取消修改").text("修改");
                var e = '<div class="form-style-1-field urgentphone" style="float: left;width: 100%;padding-left: 245px;color: #b73736;line-height: 2.8;">设置紧急联系人前请先完成手机认证</div>';
                return $(".urgentphone").length || $("#phoneCon1").find(".form-style-1-submit").after(e),
                safe.phone.bind(safeData),
                safe.bindedObj.phone = 1,
                safe.phone.open(),
            ...
            $w.slideDown(),
            "none" == $w.find(".form-style-1-field").css("display") && ($w.find(".form-style-1-field").show().end().find(".form-style-1-submit").show(), $("#urgentSucceed").hide()),
            safe.manageObj.urgent = 1,
            $w.find(".name-error").text(""),
            $w.find(".phone-error").text(""),
            $w.find(".relation-error").text(""),
            $w.find(".pay-error").text(""),
            $w.find(".sub-error").text(""),
            $("#urgentBtn1").addClass("safe-off").attr("switchtext", "设置").text("取消设置"),
            $("#urgentBtn2").addClass("safe-off").attr("switchtext", "修改").text("取消修改")
        },
        close: function() {
            var $w = $("#urgentWrapper"),
            safe = itz.safe;
    ...
    GoStep: function($spanArray, num) {
        ...
        $spanArray.each(function(index, ele) {
            return $ele = $(ele),
            index > num - 1 && $ele.hasClass("options-step-current") ? ($ele.removeClass("options-step-current"), !0) : void(num - 1 >= index && !$ele.hasClass("options-step-current") && $ele.addClass("options-step-current"))
        })
    },
    ...
    getInfoByIdCard: function(sId, serverTime) {
        ...
        if18 = !1;
        return curTime.getFullYear() >= d.getFullYear() + 18 && (curTime.getFullYear() == d.getFullYear() + 18 ? curTime.getMonth() >= d.getMonth() && (curTime.getMonth() == d.getMonth() ? curTime.getDate() >= d.getDate() ? if18 = !0 : curTime.getMonth() + 1 == 2 && 29 == d.getDate() && (if18 = !0) : if18 = !0) : if18 = !0),
        {
            code: 0,
            data: {
                birthday: sBirthday,
