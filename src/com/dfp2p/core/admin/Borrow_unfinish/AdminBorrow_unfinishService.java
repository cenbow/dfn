...
@Service("adminBorrow_unfinishService")
public class AdminBorrow_unfinishService{

	private static Log logger = LogFactory.getLog(AdminBorrow_unfinishService.class);
	
	@Resource(name="adminBorrow_unfinishDao")
	private AdminBorrow_unfinishDao thisDao;
	...
	public HashMap<String, Object> getBorrow_unfinishList(PageData pd,
			ModelAndView mv) 
		{
		...
		whereUtil.getStringCondition("borrow_status", "3", "=");
		...
		try{
			...
			list = thisDao.getBorrow_unfinishList(conn, rp);
		} catch (Exception e) {
