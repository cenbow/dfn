...
@Service("adminBorrow_repaymentService")
public class AdminBorrow_repaymentService{

	private static Log logger = LogFactory.getLog(AdminBorrow_repaymentService.class);
	
	@Resource(name="adminBorrow_repaymentDao")
	private AdminBorrow_repaymentDao thisDao;
	...
	public HashMap<String, Object> getBorrow_repaymentList(PageData pd,
			ModelAndView mv) 
		{
		...
		whereUtil.getStringCondition("borrow_status", "6", "=");
		HashMap<String,String> defaultMap =  whereUtil.getCommonMap();
		
		rp.put("paramCondition",whereUtil.getParamConditionList());
		mv.addObject("defaultMap",defaultMap);
		
		if(DfMath.toIntValue(pd.get("lastDay"))>0){
			whereUtil.getStringCondition("FROM_UNIXTIME( bi.`deadline`, '%d' )",pd.get("lastDay"),"=");
			defaultMap.put("lastDay", pd.get("lastDay"));
		}
		...
		try{
			...
			list = thisDao.getBorrow_repaymentList(conn, rp);
		} catch (Exception e) {
